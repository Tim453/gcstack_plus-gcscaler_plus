...
#ifndef TRACE_PARSER_LEGACY_H
#define TRACE_PARSER_LEGACY_H
...
struct inst_memadd_info_t {
  uint64_t addrs[LEGACY_WARP_SIZE];
  int32_t width;
...
struct inst_trace_t {
  ...
  // Raw 64-bit SASS control word (first trace column, tracer version >= 6).
  // 0 when the trace predates control-word logging. Decoded into the
  // STALL/YIELD/dependency-barrier fields by DecocdeInstLegacy.
  unsigned long long control_word = 0;
