// Legacy text-based trace parser, wrapped in namespace to coexist with
// the protobuf-based enhanced parser.
...
bool inst_trace_t::parse_from_string(std::string trace,
  ...
  if (trace_version >= 6){
    // First column is the raw 64-bit SASS control word. Keep it so the decoder
    // can extract STALL/YIELD/dependency-barrier fields (control-bit model).
    ss >> std::hex >> control_word;
  }

  ss >> std::hex >> m_pc;
  ss >> std::hex >> mask;

  std::bitset<LEGACY_WARP_SIZE> mask_bits(mask);
...
kernel_trace_t *trace_parser::parse_kernel_info(
  ...
  if (!(kerneltraces_filepath.rfind(".gz") != std::string::npos ||
        kerneltraces_filepath.rfind(".xz") != std::string::npos))
  {
    std::cout << "File is not compressed: " << kerneltraces_filepath << std::endl;
    exit(1);
  }
  kernel_info->ifs = new bxz::ifstream;
  bxz::ifstream *ifs = kernel_info->ifs;
...
void trace_parser::get_next_threadblock_traces(
    ...
    unsigned trace_version, bxz::ifstream *ifs) {
  ...
  while (!ifs->eof()) {
    ...
    if (line.length() == 0) {
      ...
      if (string1 == "#BEGIN_TB") {
        ...
        inst_count = 0;
      } else if (string1 == "sm_id") {
        int smid = 0;
        sscanf(line.c_str(), "sm_id = %d", &smid);
      } else{
        assert(start_of_tb_stream_found);
