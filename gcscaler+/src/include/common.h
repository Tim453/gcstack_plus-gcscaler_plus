...
namespace GCoM
{
    ...
    struct HWConfig
    {
        ...
        std::string wrapSchedulePolicy = "gto"; // [gto: greedy then oldest | lrr: loosely round-robin | gty/cggty: greedy then youngest]
        ...
        unsigned sharedMemLatency = 30; // L1S (local CTA shared memory)
        ...
        // L2 cache (used by simple cache model)
        ...
        // L1D set hash function (used by simple cache model)
        ...
        unsigned kernelLaunchOverhead = 0;

        // Scale the NoC and DRAM queueing delay by the fraction of the interval window the
        // offered load actually occupies, instead of charging the full burst service time of
        // every active SM. Keeps the queueing term from growing linearly with the SM count
        // on designs where the link is not saturated.
        bool memContentionUtilGate = false;
    };
    ...
    struct KernelInfo
    {
        ...
        unsigned clusterDimX = 1;
        unsigned clusterDimY = 1;
        unsigned clusterDimZ = 1;
    };
