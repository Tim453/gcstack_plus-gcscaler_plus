...
int GCoM::RunGCoMWithAccelsimCacheModelGPUMechRepWarpSelect(OptionParser opp)
{
    ...
    std::ofstream myfile;
    myfile.open (opp.mCsvPath);
    myfile << "Kernel,numThreadInst,numCycle,cpi,base,comData,comStruct,memData,memStruct,idle\n";
    cout << "Kernel,numThreadInst,numCycle,cpi,base,comData,comStruct,memData,memStruct,idle\n";
    for (long unsigned int i = 0; i < eachKernelResults.size(); i++)
    {
        myfile << i << "," << eachKernelResults[i] << endl;
        cout << i << ",";
        cout << eachKernelResults[i] << endl;
    }
...
{
    // Register dependencies are reconstructed from a simulated scoreboard (CheckSrcDep).
...
std::ostream& operator<<(std::ostream &os, const GCoMModel::Result &result)
{
    ...
    os << result.numThreadInst << "," << result.numCycle << "," << result.cpi << "," << result.cpiBase << "," << result.cpiComData << "," << result.cpiComStruct << "," << result.cpiMemData << "," << result.cpiMemStruct << "," << result.cpiIdle;
...
int GCoMModel::RunPerformanceModel(KernelInfo kernelInfo, bool isGCStackIdleDef, Result &result)
{
    ...
    GPUCycleBreakdown.tot += 1 + mHWConfig.kernelLaunchOverhead;
    GPUCycleBreakdown.idle += 1 + mHWConfig.kernelLaunchOverhead;
...
int GCoMModel::RunPerSMModel(unsigned allocedCTAPerSM, unsigned maxConcurrentCTAPerSM, unsigned numWarpsPerCTA, unsigned numActiveSM,
        CycleBreakdown &smCycleBreakdown)
{
    int error = 0;
...
int GCoMModel::RunPerIntervalModel(unsigned intervalIdx, unsigned smConcurrentWarps, unsigned numActiveSM,
        CycleBreakdown &smCycleBreakdown)
{
                ...
                double issueRate, double numOthW, double &othBase, double &dataStallSub, double &issuedInstnInStall)
{
    ...
    else if (warpSchedulePolicy == "gty" || warpSchedulePolicy == "cggty")
    {
        // GTY (Greedy Then Youngest / CGGTY): the greedy warp runs like GTO, but when it
        // stalls the scheduler picks the warp with the highest dynamic ID (most recently
        // dispatched). The youngest warp has fewer accumulated memory stalls and is more
        // likely to have a ready instruction than GTO's oldest fallback.
        //
        // othBase: greedy mechanism → average interval instructions, same as GTO.
        //
        // issuedInstnInStall: the youngest warp is only exercised during data
        //   stalls, so use the uncapped mIssueProb as its readiness.
        othBase = numOthW * mIntervalProfile.mAvgIntvWarpInsts / issueRate;
        issuedInstnInStall = numOthW * mIntervalProfile.mIssueProb;
    }
    else
    {
        cout << "[Error] Unsupported warp schedule policy: " << warpSchedulePolicy << endl;
        return -1;
    }
                ...
                CycleBreakdown &intvCycleBreakdown)
{
    ...
    double nocUtil = 1.0, dramUtil = 1.0;
    if (mHWConfig.memContentionUtilGate)
    {
        double window = max(intvCycleBreakdown.tot, 1.0);
        nocUtil = min(1.0, numActiveSM * (rMiss + wMiss) * nocSerLat / window);
        dramUtil = min(1.0, numActiveSM * missDram * dramSerLat / window);
    }
            ...
            (nocSerLat * (rMiss + wMiss) * numActiveSM > (double) mHWConfig.L2Latency + mHWConfig.dramLatency))
    {
        ...
        memDataQueueNoC = nocUtil * numActiveSM * (rMiss + wMiss) * nocSerLat;
        memDataQueueDRAM = dramUtil * numActiveSM * missDram * dramSerLat;

        // for each MSHR batching no contention read latency + NoC + DRAM queing delay occurs,
        // but write queuing delay occur only once.
        mshrBatchLatency = noContentionLatency + nocUtil * numActiveSM * rMiss * nocSerLat +
                dramUtil * numActiveSM * rMiss * l2RMissRate * dramSerLat;
    }
    else
    {
        memDataQueueNoC = 0.5 * nocUtil * numActiveSM * (rMiss + wMiss) * nocSerLat;
        memDataQueueDRAM = 0.5 * dramUtil * numActiveSM * missDram * dramSerLat;

        // for each MSHR batching no contention read latency + NoC + DRAM queing delay occurs,
        // but write queuing delay occur only once.
        mshrBatchLatency = noContentionLatency + 0.5 * (nocUtil * numActiveSM * rMiss * nocSerLat +
                dramUtil * numActiveSM * rMiss * l2RMissRate * dramSerLat);
    }
